...
export default function MediaPage() {
  ...
  const fetchMedia = () => {
    setLoading(true);
    api.get<PaginatedResponse<MediaItem>>("/media", { limit: 100, sort: "createdAt", order: "desc" })
      .then(setData)
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-white">
        {loading ? (
          ...
          <div className="grid grid-cols-3 gap-3 p-4 sm:grid-cols-4 sm:gap-4 sm:p-6 md:grid-cols-5 lg:grid-cols-6">
            {data.data.map((item) => (
              <div key={item.id} className="group relative aspect-square overflow-hidden rounded-lg border border-line bg-bg2">
