...
export default function CartPage() {
  const { items, total, shipping, updateQuantity, removeItem } = useCart();
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2">
            {items.map((item) => (
              <div key={item.slug} className="flex gap-4 border-b border-line py-4 last:border-0">
          ...
          <div className="lg:col-span-1">
            <div className="rounded-xl border border-line bg-white p-6 shadow-panel">
              ...
              <div className="space-y-3 text-sm">
                ...
                {shipping > 0 && (
                  <p className="text-xs text-ink-3">Free shipping on orders over AED 500</p>
                )}
                <div className="border-t border-line pt-3">
                  <div className="flex justify-between text-base font-semibold">
                    <span className="text-ink">Total</span>
                    <span className="text-ink">AED {grandTotal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              <Link href="/checkout">
